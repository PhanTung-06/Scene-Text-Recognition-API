...
class Detector:
    def __init__(self, cfg_path, model_weight_path):
        ...
        self.vis_text = self.cfg.MODEL.ROI_HEADS.NAME == "TextHead"
    ...
    def predict(self, image):
        ...
        image = image[:, :, ::-1]
        if self.vis_text:
            visualizer = TextVisualizer(image, self.metadata, instance_mode=ColorMode.IMAGE)
        else:
        ...
        # cv2.imwrite('result.jpg',vis_output)
        vis_output.save('result.jpg')
        # cv2.imshow('result', vis_output.get_image()[:, :, ::-1])
